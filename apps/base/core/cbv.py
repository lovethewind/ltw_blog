...
def _init_cbv(cls: Type[Any], instance: Any = None) :
    ...
    * The `__signature__` attribute is updated to indicate to FastAPI what arguments should be passed to the initializer
    """
    from .depend_inject import Autowired, Value
    ...
    for name, hint in get_type_hints(cls).items():
        val = getattr(cls, name, Ellipsis)
        if is_classvar(hint):
            continue
        if isinstance(val, (Autowired, Value)):  # 跳过Autowired、Value
            continue
...
def _register_endpoints(router: APIRouter, cls: Type[T], *urls: str) :
    ...
    for route in router.routes:
        if not isinstance(route, (APIRoute, WebSocketRoute)):  # 修改使兼容WebSocketRoute
            raise ValueError("The provided routes should be of type APIRoute/WebSocketRoute")
        if isinstance(route, APIRoute):  # 修改使兼容WebSocketRoute
            route_methods: Any = route.methods
            cast(Tuple[Any], route_methods)
            router_roles.append((route.path, tuple(route_methods)))
        else:
            router_roles.append((route.path,))
...
def _update_cbv_route_endpoint_signature(cls: Type[T], route: Union[Route, WebSocketRoute]) :
    ...
    """
    from .depend_inject import BeanFunc

    old_endpoint = route.endpoint
    old_signature = inspect.signature(old_endpoint)
    old_parameters: List[inspect.Parameter] = list(old_signature.parameters.values())
    old_first_parameter = old_parameters[0]
    # 此处将self参数从Depends每次实例化替换为获取单例
    new_first_parameter = old_first_parameter.replace(default=Depends(BeanFunc(cls)))
